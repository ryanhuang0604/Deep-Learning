...
EOS_token = 1
#----------Hyper Parameters----------#
# The number of vocabulary: input_size==vocab_size  {SOS,EOS,a,b,c,....,z}
input_size = 28
...
if __name__ == '__main__':
    ...
    # VAE model
    vae = VAE(input_size, hidden_size, latent_size, conditional_size,
              max_length=dataset_train.max_length).to(device)
    ...
    for epoch in range(1, epochs+1):
        ...
        CEloss, KLloss, _ = train(vae, loader_train, optimizer,
                                  teacher_forcing_ratio, kl_weight, dataset_train.tensor2string)
        ...
        print(
            f'epoch{epoch:>2d}/{epochs}  tf_ratio:{teacher_forcing_ratio:.2f}  kl_weight:{kl_weight:.2f}')
        print(f'CE:{CEloss:.4f} + KL:{KLloss:.4f} = {CEloss+KLloss:.4f}')

        """
        evaluate
        """
        conversion, BLEUscore = evaluate(
            vae, loader_test, dataset_test.tensor2string)
        # generate words
        ...
        if BLEUscore > best_BLEUscore:
            ...
            torch.save(best_model_wts, os.path.join(
                'models', f'{kl_annealing_type}_time{time}_epochs{epochs}.pt'))
            fig = plot(epoch, CEloss_list, KLloss_list, BLEUscore_list,
                       teacher_forcing_ratio_list, kl_weight_list)
            fig.savefig(os.path.join(
                'results', f'{kl_annealing_type}_time{time}_epochs{epochs}.png'))

    torch.save(best_model_wts, os.path.join(
        'models', f'{kl_annealing_type}_time{time}_epochs{epochs}.pt'))
    fig = plot(epochs, CEloss_list, KLloss_list, BLEUscore_list,
               teacher_forcing_ratio_list, kl_weight_list)
    fig.savefig(os.path.join(
        'results', f'{kl_annealing_type}_time{time}_epochs{epochs}.png'))
