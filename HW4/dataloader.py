    ...
    def get_dataset(self, path, is_train):
        ...
        with open(path, 'r') as file:
            if is_train:
                ...
                test_tenses = [['sp', 'p'], ['sp', 'pg'], ['sp', 'tp'], ['sp', 'tp'], [
                    'p', 'tp'], ['sp', 'pg'], ['p', 'sp'], ['pg', 'sp'], ['pg', 'p'], ['pg', 'tp']]
                for test_tense in test_tenses:
                    tenses.append([self.tense2idx[tense]
                                   for tense in test_tense])
        return words, tenses
...
class MyDataSet(data.Dataset):
    def __init__(self, path, is_train):
        ...
        self.words, self.tenses = self.dataTransformer.get_dataset(
            os.path.join('data', path), is_train)
        self.max_length = self.get_max_length(self.words)
        # output=(time1,1) tensor
        self.string2tensor = self.dataTransformer.string2tensor
        # output=(1) tensor
        self.tense2tensor = self.dataTransformer.tense2tensor
        # input=(time1,1) tensor
        self.tensor2string = self.dataTransformer.tensor2string
        assert len(self.words) == len(
            self.tenses), 'word list is not compatible with tense list'
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            return self.string2tensor(self.words[idx][0], add_eos=True), self.tense2tensor(self.tenses[idx][0]),\
                self.string2tensor(self.words[idx][1], add_eos=True), self.tense2tensor(
                    self.tenses[idx][1])
