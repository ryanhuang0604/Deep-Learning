...
class ReplayMemory:
	...
	def sample(self, batch_size, device):
		...
		return (torch.tensor(x, dtype=torch.float, device=device) for x in zip(*transition))
...
class DDPG:
	...
	def select_action(self, state, noise=True):
		...
		with torch.no_grad():
			if noise:
				action = self._actor_net(torch.from_numpy(state).view(1, -1).to(self.device)) + torch.from_numpy(self._action_noise.sample()).view(1, -1).to(self.device)
			else:
...
def test(args, env, agent, writer):
	...
	for n_episode, seed in enumerate(seeds):
		...
		for t in itertools.count(start=1):
			...
			if done:
				writer.add_scalar('Test/Episode Reward', total_reward, n_episode)
				print("ep {} | Total reward: {}".format(n_episode, total_reward))
