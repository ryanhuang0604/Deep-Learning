...
class CLEVRDataset(Dataset):
    def __init__(self, img_path, json_path):
        ...
        self.max_objects = 0
        with open(os.path.join('data', 'objects.json'), 'r') as file:
        ...
        self.transformations = transforms.Compose([transforms.Resize(
            (64, 64)), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    ...
    def __getitem__(self, index):
        img = Image.open(os.path.join(
            self.img_path, self.img_names[index])).convert('RGB')
